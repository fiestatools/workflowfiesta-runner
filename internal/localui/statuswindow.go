...
func NewStatusWindow(runnerName, apiURL string) *StatusWindow {
	...
	// Persist window size after the user resizes it.
	// We poll every second and only save after 3 consecutive stable readings
	// to avoid spurious saves from content reflows mid-job.
	go func() {
