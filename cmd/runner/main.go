	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if headless {
			...
			updater.Start(updater.Options{
				...
				Log:          func(msg string) { log.Info(msg) },
				IsIdle:       func() bool { return r.ActiveJobCount() == 0 },
				Drain:        func() error { r.SetDraining(true); return client.SendDraining(runner.RunnerCapabilities, runtime.GOOS, runtime.GOARCH, version) },
				CancelDrain:  func() { r.SetDraining(false) },
				Quit:         func() { os.Exit(0) },
